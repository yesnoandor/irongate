...
static void qpnp_dump_regs(struct qpnp_led_data *led, u8 regs[], u8 array_size)
{
	...
	pr_err("===== %s LED register dump start =====\n", led->cdev.name);
	...
	pr_err("===== %s LED register dump end =====\n", led->cdev.name);
}
...
static int qpnp_rgb_set(struct qpnp_led_data *led)
{
	...
	if (led->cdev.brightness) {
		...
		if (led->rgb_cfg->pwm_cfg->mode == PWM_MODE) {
			...
			printk(KERN_ERR "pwm label = %s\r\n",led->rgb_cfg->pwm_cfg->pwm_dev->label);
				
			printk(KERN_ERR "period_us = %d\r\n",period_us);
		
			if (period_us > INT_MAX / NSEC_PER_USEC) {
				duty_us = (period_us * led->cdev.brightness) /
					LED_FULL;

				printk(KERN_ERR "duty_us = %d\r\n",duty_us);
		...
		printk(KERN_ERR "pwm_enabled = %d\r\n",led->rgb_cfg->pwm_cfg->pwm_enabled);
		...
		printk(KERN_ERR "mode = %d\r\n",led->rgb_cfg->pwm_cfg->mode);
		printk(KERN_ERR "default_mode = %d\r\n",led->rgb_cfg->pwm_cfg->default_mode);
		printk(KERN_ERR "pwm_enabled = %d\r\n",led->rgb_cfg->pwm_cfg->pwm_enabled);
...
static int qpnp_leds_probe(struct platform_device *pdev)
{
	...
	for_each_child_of_node(node, temp) {
		...
		printk(KERN_ERR "parsed_leds = %d\r\n",parsed_leds);
		printk(KERN_ERR "label = %s\r\n",led_label);
		printk(KERN_ERR "name = %s\r\n",led->cdev.name);
		printk(KERN_ERR "max_current = %d\r\n",led->max_current);
		printk(KERN_ERR "id = %d\r\n",led->id);
...
static int qpnp_leds_suspend(struct platform_device *pdev, pm_message_t mesg)
{
	struct qpnp_led_data *led_array = dev_get_drvdata(&pdev->dev);
	struct qpnp_led_data *led;
	...
	//printk(KERN_ERR "led_array = 0x%x\r\n",(unsigned int)led_array);

	
	led = (led_array + QPNP_ID_RGB_RED - QPNP_ID_RGB_RED);
	led->cdev.brightness = LED_OFF;
	qpnp_led_set(&led->cdev, led->cdev.brightness);

	led = (led_array + QPNP_ID_RGB_GREEN - QPNP_ID_RGB_RED);
	led->cdev.brightness = LED_DIM;
	qpnp_led_set(&led->cdev, led->cdev.brightness);

	led = (led_array + QPNP_ID_RGB_BLUE - QPNP_ID_RGB_RED);
	led->cdev.brightness = LED_OFF;
	qpnp_led_set(&led->cdev, led->cdev.brightness);
...
static int qpnp_leds_resume(struct platform_device *pdev)
{
	struct qpnp_led_data *led_array = dev_get_drvdata(&pdev->dev);
	struct qpnp_led_data *led;
...
static struct platform_driver qpnp_leds_driver = {
	...
	.suspend        = qpnp_leds_suspend,
	.resume          =  qpnp_leds_resume,
};
